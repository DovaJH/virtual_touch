...
std::vector<int> GestureController::get_raised_fingers(const LandmarkList& landmarks) {
...
void GestureController::handle_gestures(const LandmarkList& landmarks) {
    if (landmarks.size() < 9) return;

    std::vector<int> fingers = get_raised_fingers(landmarks);
    ...
    if (is_drag_gesture) {
        if (!mouse_hold_state_) {
            mouse_controller_.press(1); // Left button down
        ...
        if (mouse_hold_state_) {
            mouse_controller_.release(1); // Left button up
