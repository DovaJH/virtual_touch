...
class GestureRecognizer:
    def __init__(self, scr_w, scr_h):
        ...
        # 기본 변수 설정
        self.frameR = 150  # 프레임 경계
        self.smooth_alpha = 0.3  # 마우스 이동 부드러움 정도 조절
    ...
    def is_fingers_raised(self):
        ...
        # 엄지 확인
        if self.landmark_list[self.fingertip_index[0]][1] > self.landmark_list[self.fingertip_index[0] - 2][1]:
            fingers.append(1)
        else:
            fingers.append(0)
        ...
        # 나머지 손가락들 확인
        for i in range(1, 5):
            if self.landmark_list[self.fingertip_index[i]][2] < self.landmark_list[self.fingertip_index[i] - 3][2]:
                fingers.append(1)
            else:
                fingers.append(0)
    ...
    def mouse_event(self, fingers, prev_x, prev_y):
        ...
        if fingers[0] == 0 and fingers[1] == 1 and fingers[2] == 0 and fingers[3] == 0 and fingers[4] == 0:
            # 선형 보간으로 좌표 보정
            new_index_finger_x = np.interp(index_finger_x, (self.frameR, self.w - self.frameR), (0, self.scr_w))
            new_index_finger_y = np.interp(index_finger_y, (self.frameR, self.h - self.frameR), (0, self.scr_h))
